...
SELECT año,genero, COUNT(*) AS cantidad_peliculas
FROM moviedatabase
WHERE tipo = 'movie'
GROUP BY año
...
SELECT año, genero,  COUNT(*) AS cantidad_cortos
FROM moviedatabase
WHERE tipo = 'short'
GROUP BY año
...
GROUP BY m.puntuacion
HAVING puntuacion NOT LIKE "No info"
ORDER BY m.puntuacion DESC
LIMIT 10;

-- 6.¿Cuál es la película mejor valorada en IMDB?
SELECT titulo, director, MAX(puntuacion) AS Mejor_Puntuacion
FROM imdb 
GROUP BY puntuacion
HAVING puntuacion NOT LIKE "No info"
...
SELECT nombre_apellido, premios 
FROM actores
WHERE premios 
